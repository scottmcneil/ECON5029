...
Our augmented model will be as follows:
...
$\hat{S}_t$, $\Delta R_t$ and $-\Delta FFF_t$ are the same as in our baseline model and $\hat{F}_t$ is a $k \times 1$ vector of estimated real-economic factors at time $t$. $u_{t,S}$ is the $m\times1$ vector of structural innovations in $\hat{S}_t$ at time $t$ and $u_{t,F}$ is $k \times 1$ vector of structural innovations in $\hat{F}_t$. $u_{t,FFF}$ and $u_{t,R}$ are the structural innovations in $-\Delta FFF_t$ and $\Delta R_t$, respectively, at time $t$. $A_f$ and $B_f$ are square coefficient matrices of dimension $k + m + 2$. Our transformed model will then be:
...
Where $C_f = A_f^{-1}B_f$ and $v_{t,S}$, $v_{t,R}$, $v_{t,FFF}$ and $v_{t,F}$ are the transformed versions of $u_{t,S}$, $u_{t,R}$, $u_{t,FFF}$ and $u_{t,F}$, respectively.
